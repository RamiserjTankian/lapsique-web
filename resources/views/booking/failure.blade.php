...
@section('title', 'Pago no completado — lapsique.media')
...
<div class="min-h-[70vh] flex items-center justify-center py-16">
    <div class="max-w-lg w-full space-y-8 text-center fade-up">
        <div class="mx-auto flex items-center justify-center w-24 h-24 rounded-full bg-red-500/20 border border-red-400/40">
        ...
        <div class="space-y-3">
            <h1 class="text-3xl font-bold text-white">Pago no completado</h1>
            <p class="text-gray-300">Hubo un problema al procesar tu pago. No se realizó ningún cargo.</p>
        </div>

        <div class="space-y-3">
            <div class="flex flex-wrap justify-center gap-3">
                <a href="{{ route('booking.show') }}" class="btn btn-primary inline-flex">
                    Intentar de nuevo
                </a>
                <a href="{{ route('customers.portal') }}" class="btn btn-ghost inline-flex">
                    Ver mi portal
                </a>
            </div>
            ...
            @if($whatsapp)
            <div>
                <p class="text-sm text-gray-400 mb-3">¿Necesitas ayuda? Contáctanos directamente:</p>
                <a href="https://wa.me/{{ $whatsapp }}?text=Hola%2C+intente+reservar+una+sesi%C3%B3n+de+contenido+pero+el+pago+no+se+complet%C3%B3.+Me+pueden+ayudar%3F"
...
<script>
(function () {
    const key = 'booking_failed_fired_{{ $booking->public_id }}';
    ...
    if (typeof window.LapsiqueTracker !== 'undefined') {
        window.LapsiqueTracker.track('booking_payment_failed', {
