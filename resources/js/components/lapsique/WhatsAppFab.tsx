...
const PROMPT_MESSAGES = [
    'Haz tu producción realidad.',
    'Cuéntanos tu proyecto por WhatsApp.',
    'Reserva tu sesión en minutos.',
];

const DEFAULT_WHATSAPP_TEXT = 'Hola, me interesa lapsique.media';
...
export function WhatsAppFab() {
    ...
    const href = useMemo(() => {
        ...
        return `https://wa.me/${number}?text=${encodeURIComponent(DEFAULT_WHATSAPP_TEXT)}`;
    }, [number]);

    const { displayed, showCursor } = useTypingCycle({
        texts: PROMPT_MESSAGES,
        enabled: isPromptVisible && !prefersReducedMotion,
    });

    const promptText = prefersReducedMotion ? PROMPT_MESSAGES[0] : displayed;
